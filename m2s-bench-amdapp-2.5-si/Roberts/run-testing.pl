...
foreach $item (@benchs)
{
	...
	foreach $fd (@fdist){
		...
		system("~/approximate_associative_mem_gpu/multi2sim-4.2/bin/m2s --si-sim detailed --si-fifo-length $length --si-profiling 0 --si-hamming-dist $dist --si-tcam-dist $fd --si-config conf Roberts --load Roberts_Kernel.bin ");
