...
foreach $item (@benchs)
{
    foreach $length (@fifo_list)
    {
        ...
        system("~/approximate_associative_mem/multi2sim-4.2/bin/m2s --si-sim detailed --si-fifo-length $length --si-profiling $length --si-config conf Roberts --load Roberts_Kernel.bin");
