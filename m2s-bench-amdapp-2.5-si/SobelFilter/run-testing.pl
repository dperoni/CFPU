...
system("~/approximate_associative_mem_gpu/multi2sim-4.2/bin/m2s --si-sim detailed --si-fifo-length 0 --si-profiling 0 --si-hamming-dist $dist --si-tcam-dist $fd --si-abuff-size $bf  --si-config conf SobelFilter --load SobelFilter_Kernels.bin > hit.rep");
