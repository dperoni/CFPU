...
foreach $item (@folder_lst)
{
    ...
    foreach $length (@fifo_list)
    {
        ...
        system("convert ./images/0.jpg -resize 256x256! ./images/0.bmp");
        system("m2s --si-sim detailed --si-fifo-length $length --si-profiling 0 --si-hamming-dist $dist --si-config conf RecursiveGaussian --load RecursiveGaussian_Kernels.bin > hit.rep");
