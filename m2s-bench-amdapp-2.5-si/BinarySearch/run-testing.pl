...
@folder_lst = (5000, 5100);


@fifo_list = (64); #16, 32, 64, 128, 256, 512, 1024);
$dist = 0;
  ...
  foreach $item (@folder_lst)
	...
	foreach $fd (@fdist){
		...
		#system("~/approximate_associative_mem/multi2sim-4.2/bin/m2s --si-sim detailed --si-fifo-length $length --si-profiling 0 --si-tcam-dist $fdist --si-config conf DwtHaar1D --load DwtHaar1D_Kernels.bin -x $item -e ");
	        system("~/approximate_associative_mem/multi2sim-4.2/bin/m2s --si-sim detailed --si-fifo-length $length --si-profiling 0 --si-tcam-dist $fd --si-config conf BinarySearch --load BinarySearch_Kernels.bin -x $item -e > hit.rep");
