...
foreach $item (@benchs)
{
    foreach $length (@fifo_list)
    {
        ...
        system("~/approximate_associative_mem_gpu/multi2sim-4.2/bin/m2s --x86-debug-loader stdout --si-sim detailed --si-fifo-length $length --si-profiling $length --si-config conf Sharpen --load Sharpen_Kernel.bin");
