...
#        copy("../Caltech/$item/image_0001.jpg", "./images/0.jpg");
  #      system ("convert ./images/0.jpg -resize 256 ./images/0.bmp");
        system("~/approximate_associative_mem_gpu/multi2sim-4.2/bin/m2s --si-sim detailed --si-fifo-length $length --si-profiling $length  --si-config conf nn ./inputs -p 0 -d 1");
