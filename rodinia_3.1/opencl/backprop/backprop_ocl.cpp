...
int bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
	...
	for (int j = 1; j <= hid; j++) {
		...
		printf("Output %d\n", (int)(10000*partial_sum[k * hid + j-1]));
    }
	...
	bpnn_output_error(net->output_delta, net->target, net->output_units, out, eo/*&out_err*/);
	bpnn_hidden_error(net->hidden_delta, hid, net->output_delta, out, net->hidden_weights, net->hidden_units, /*&hid_err*/eh);  
	...
	/*int jj;
	for(jj = 0; jj < in*hid; jj++){
			printf("Out; %d\n", (int)(10000*input_weights_one_dim[jj]));
...
/*	int j, k;
	for (j = 1; j <= out; j++) {
	    for (k = 0; k <= hid; k++) {
		printf("Out %d\n", (int)(100000000*net->hidden_weights[k][j]));
	    }
	  }*/
