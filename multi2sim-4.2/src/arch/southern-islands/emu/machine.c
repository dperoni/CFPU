...
#define INST SI_INST_SOP2
void si_isa_S_SUB_U32_impl(struct si_work_item_t *work_item,
	struct si_inst_t *inst)
{
	union si_reg_t s0;
	union si_reg_t s1;
	...
	struct si_inst_t *inst)
{
	...
	if(CFPU_MODE == EXACT){
		...
		/*If either is 0, set result to 0*/	
		if(s0.as_float == 0 || s1.as_float == 0){
			product.as_float = 0.0f;
			FPU_hit_cnt[si_isa_V_MUL_F32] ++;
		}
		else if(CFPU_MODE == MANTISSA_DROP){ //Just pick one and go
			product.as_float = sign*ldexp(sig1, (exp0+exp1-1));
			FPU_hit_cnt[si_isa_V_MUL_F32] ++;
		}
		/*Otherwise discard a power of 2 if possible*/
			...
			//printf("Here %f %f, %f %f\n", sig0,  sig1, s0.as_float, s1.as_float);
			/*Find the mantissa producing the best result and switch values if needed*/
			if (CFPU_MODE != ADAPTIVE_SIMPLE_TUNING && CFPU_MODE != ADAPTIVE_SIMPLE){
				int temp_ex0 = sig0 > 0.75;
				int temp_ex1 = sig1 > 0.75;
				float tempf0 = sign*ldexp(sig1, (exp0+exp1-1+temp_ex0));
				float tempf1 = sign*ldexp(sig0, (exp0+exp1-1+temp_ex1));
				if(abs(tempf0 - s0.as_float*s1.as_float) > abs(tempf1 - s0.as_float*s1.as_float)){		
			...
			if(CFPU_MODE == ADAPTIVE || CFPU_MODE == ADAPTIVE_SIMPLE){
				product.as_float = sign*ldexp(sig1, (exp0+exp1-1+extra));
				FPU_hit_cnt[si_isa_V_MUL_F32] ++;
			}
			else if(CFPU_MODE == TUNING || CFPU_MODE == ADAPTIVE_SIMPLE_TUNING){
				if(distance < CFPU_MAX_ERROR){
					product.as_float = sign*ldexp(sig1, (exp0+exp1-1+extra));
					FPU_hit_cnt[si_isa_V_MUL_F32] ++;
				}
				else{
			...
			else if (CFPU_MODE >= SHIFT_AND_ADD){
				if(distance > CFPU_MAX_ERROR){

				float second_stage_error = CFPU_MAX_ERROR;
				if(CFPU_MODE == SHIFT_AND_ADD){
					second_stage_error = 1;
				...
				if(second_bit_0 == -1){
					...
					if(pos0 > pos1){
						...
						if(0.5/(float)(1<<pos0) <= second_stage_error){
							int pos = 24 - first_bit_0;
							int placeholder = (int)(sig1*(65536.0));
							placeholder >>= pos;
							float new_float = ((float)placeholder)/((float)(65536.0));
						...
						if(0.5/(float)(1<<pos1) <= second_stage_error){
							int pos = 24 - first_bit_1;
							int placeholder = (int)(sig0*(65536.0));
							placeholder >>= pos;
							float new_float = ((float)placeholder)/((float)(65536.0));
	...
	struct si_inst_t *inst)
{
	...
	if (addr.as_uint > MIN(work_item->work_group->ndrange->local_mem_top, 
		si_isa_read_sreg(work_item, SI_M0)))
	{
		fatal("%s: invalid address, 0x%X, 0x%X\n", __FUNCTION__, addr.as_uint, MIN(work_item->work_group->ndrange->local_mem_top, si_isa_read_sreg(work_item, SI_M0)));
	}
...
void si_isa_BUFFER_LOAD_UBYTE_impl(struct si_work_item_t *work_item,
	struct si_inst_t *inst)
{
	assert(!INST.addr64);
	assert(!INST.glc);
