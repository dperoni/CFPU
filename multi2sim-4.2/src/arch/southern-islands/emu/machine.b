...
int hamm_dis (float a, float b)
{
    ...
    for (unsigned int i = 0; i < 31; i=i+2)
    {
        if ((((1 << i) & f2int_a.output) != ((1 << i) & f2int_b.output)) ||
           (((1 << (i + 1)) & f2int_a.output) != ((1 << (i + 1)) & f2int_b.output)))
            distance++;
    }
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
    _ABS_si_isa_check_v2ops (work_item, INST.vdst, value.as_uint, (float) s0.as_int, 0.0f, si_isa_V_CVT_F32_I32, VREG);
#else  
	si_isa_write_vreg(work_item, INST.vdst, value.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
    _ABS_si_isa_check_v2ops (work_item, INST.vdst, value.as_uint, (float) s0.as_uint, 0.0f, si_isa_V_CVT_F32_U32, VREG);
#else  
	si_isa_write_vreg(work_item, INST.vdst, value.as_uint);
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
        _ABS_si_isa_check_v2ops (work_item, INST.vdst, value.as_uint, s0.as_float, 0.0f, si_isa_V_CVT_U32_F32, VREG);
#else  
	si_isa_write_vreg(work_item, INST.vdst, value.as_uint);
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
    _ABS_si_isa_check_v2ops (work_item, INST.vdst, value.as_uint, s0.as_float, 0.0f, si_isa_V_CVT_I32_F32, VREG);
#else  
	si_isa_write_vreg(work_item, INST.vdst, value.as_uint);
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
     _ABS_si_isa_check_v2ops (work_item, INST.vdst, rcp.as_uint, s0.as_float, 0.0f, si_isa_V_RCP_F32, VREG);
#else  
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_RCP_F32] ++;
    //printf ("si_isa_V_RCP_F32\n");
#endif
}
#undef INST
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
    _ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, 0.0f, si_isa_V_SIN_F32, VREG);
#else  
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt [si_isa_V_SIN_F32] ++;
    //printf ("si_isa_V_SIN_F32\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
    _ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, 0.0f, si_isa_V_COS_F32, VREG);
#else  
    si_isa_write_vreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt [si_isa_V_COS_F32] ++;
    //printf ("si_isa_V_COS_F32\n");
#endif
...
#ifdef _ABS_APPROX
	//printf("bad\n");
    float src0 = s0.as_float;
    float src1 = s1.as_float;

    if (PROFILING > 0) {
        bool find = false;
        for (unsigned int i = 0; i < fifo_length; i++) {
            if ((add[i].op0 == src0 && add[i].op1 == src1) || (add[i].op0 == src1 && add[i].op1 == src0)) {
                find = true;
                add[i].cnt ++;
                break;
            }
        ...
        if (!find) {
            if (add_index > fifo_length)
                fprintf (file_unmapped, "ADD fifo is out of space ...\n");
            else {
                add[add_index].op0 = src0;
                add[add_index].op1 = src1;
                add[add_index].q = sum.as_float;
                add[add_index].cnt = 1;
                add_index++;
            }
        }
    }
    else { // Search for loaded TCAM
        for (unsigned int i = 0; i <= add_index; i++) {
            if ((hamm_dis(add[i].op0, src0) + hamm_dis(add[i].op1, src1) <= HAMMING_THR) ||
                (hamm_dis(add[i].op0, src1) + hamm_dis(add[i].op1, src0) <= HAMMING_THR)){
                sum.as_float = add[i].q;
                FPU_hit_cnt[si_isa_V_ADD_F32] ++;
                break;
            }
        }
	...
	struct si_inst_t *inst)
{
	...
	int s_0;
	int s_1;
	int first_bit_pos, second_bit_pos;
	int i;
	int shift_val = 16;
	int extra = 0;
	float distance = 0.0f;
...
//DC
	int exp0, exp1;
	float sig0 = frexp(s0.as_float, &exp0);
	float sig1 = frexp(s1.as_float, &exp1);
	int sign0 = (sig0 < 0) ? -1 : 1;
	int sign1 = (sig1 < 0) ? -1 : 1;
	int sign = sign0*sign1;
	 //printf("%.16f %.16f\n", sig0, sig1);
	sig1 = fabsf(sig1);
	sig0 = fabsf(sig0);
	//printf("Values %f %f, %f %f\n", sig0,  sig1, s0.as_float, s1.as_float);
	if(s0.as_float == 0 || s1.as_float == 0){
		product.as_float = 0.0f;
		FPU_hit_cnt[si_isa_V_MUL_F32] ++;
	}
	else if(fabsf(sig0) == 0.5f){
		//product.as_float = s0.as_float*s1.as_float;
		product.as_float = sign*ldexp(sig1, (exp0+exp1-1));
		FPU_hit_cnt[si_isa_V_MUL_F32] ++;
	}
	else if(fabsf(sig1) == 0.5f){
		//product.as_float = s0.as_float*s1.as_float;
		product.as_float = sign*ldexp(sig0, (exp0+exp1-1));
		FPU_hit_cnt[si_isa_V_MUL_F32] ++;
	}
	else{
		
		//printf("Here %f %f, %f %f\n", sig0,  sig1, s0.as_float, s1.as_float);
		if(fabs(sig0-0.75) > fabs(sig1-0.75)){
			//printf("Before %f, %f\n", sig0, sig1);
			float temp = sig1;
			sig1 = sig0;
			sig0 = temp;
		...
		if(distance > TCAM_DIST){
			float_bits s0b;
			float_bits s1b;
			s0b.f = s0.as_float;
			s1b.f = s1.as_float;
			s0b.i = s0b.i & 0x7FFFFF;
			s1b.i = s1b.i & 0x7FFFFF;

			sig0 = fabs(frexp(s0.as_float, &exp0));
			sig1 = fabs(frexp(s1.as_float, &exp1));
			//printf("Hex %f = %X, %f = %X\n", sig0, s0b.i & 0x7FFFFF, sig1, s1b.i & 0x7FFFFF);
			...
			r = 0;
			while((s0b.i ^ (0x1 << first_bit_0-1)) >> r){
				r++;
				second_bit_0 = r;
			}
			r = 0;
			while((s1b.i ^ (0x1 << first_bit_1-1)) >> r){
				r++;
				second_bit_1 = r;
			}
			int pos0 = 24 - first_bit_0 + (first_bit_0 - second_bit_0);
			int pos1 = 24 - first_bit_1 + (first_bit_1 - second_bit_1);
			
			product.as_float = s0.as_float*s1.as_float;
			
			if(second_bit_0 == -1){
				int pos = 24 - first_bit_0;
				int placeholder = (int)(sig1*(65536.0));
				placeholder >>= pos;
				float new_float = ((float)placeholder)/((float)(65536.0));
				...
				product.as_float = sign*ldexp(sig1 + new_float, (exp0+exp1-1+extra));
				//float temp_2 = sign*ldexp(sig1, (exp0+exp1-1));
				//printf("Correct %fx%f = %f, temp %f, %d, %f\n", s0.as_float, s1.as_float, s0.as_float*s1.as_float, product.as_float, pos, new_float);
				FPU_hit_cnt[si_isa_V_MUL_F32]++;
			}
			else if(second_bit_1 == -1){
				int pos = 24 - first_bit_1;
				int placeholder = (int)(sig0*(65536.0));
				placeholder >>= pos;
				float new_float = ((float)placeholder)/((float)(65536.0));
				
				
				product.as_float = sign*ldexp(sig0 + new_float, (exp0+exp1-1));
				//float temp_2 = sign*ldexp(sig1, (exp0+exp1-1));
				//printf("Correct %f, temp %f, %f, %d, %f\n", product.as_float, temp_val, temp_2, pos, new_float);
				FPU_hit_cnt[si_isa_V_MUL_F32]++;
			}
			else{
				product.as_float = s0.as_float*s1.as_float;
				if(pos0 > pos1){
					...
					if(0.5/(float)(1<<pos0) <= TCAM_DIST){
						...
						product.as_float = sign*ldexp(sig1 + new_float, (exp0+exp1-1));
						FPU_hit_cnt[si_isa_V_MUL_F32]++;
					}
				}
				else{
					if(0.5/(float)(1<<pos1) <= TCAM_DIST){
						int pos = 24 - first_bit_1;
						int placeholder = (int)(sig0*(65536.0));
						placeholder >>= pos;
						float new_float = ((float)placeholder)/((float)(65536.0));
				...
				//printf("Hex %f = %X, first bit %d second %d, pos %d, %f = %X, first bit %d second %d, pos %d\n", sig0, s0b.i, first_bit_0, second_bit_0, pos0, sig1, s1b.i, first_bit_1, second_bit_1, pos1);
				
			}
			if(fabs(s0.as_float*s1.as_float - product.as_float) > 0.00001){
				//printf("Real: %f, calc %f\n", s0.as_float*s1.as_float, product.as_float);
				//printf("Hex %f = %X, first bit %d second %d, pos %d, %f = %X, first bit %d second %d, pos %d\n", sig0, s0b.i, first_bit_0, second_bit_0, pos0, sig1, s1b.i, first_bit_1, second_bit_1, pos1);
			}	
			//product.as_float = s0.as_float*s1.as_float;
			...
			/*if(fabs(sig1-0.75) > fabs(sig0-0.75)){
			//printf("Before %f, %f\n", sig0, sig1);
			float temp = sig1;
			sig1 = sig0;
			sig0 = temp;
			...
			product.as_float = sign*ldexp(sig0, (exp0+exp1+extra-1));
			FPU_hit_cnt[si_isa_V_MUL_F32]++;
		}
		if(sig0 == 0.0f || sig1 == 0.0f){
			//FPU_hit_cnt[si_isa_V_ADD_F32] ++;
		}
	}
...
/*
	if(fabsf(sig0) == 0.5f){
		//product.as_float = s0.as_float*s1.as_float;
		product.as_float = sign0*ldexp(sig1, (exp0+exp1-1));
		FPU_hit_cnt[si_isa_V_MUL_F32] ++;
	}
	else{
		sig1 = fabsf(sig1);
		...
		if(distance > TCAM_DIST+0.00001){
			product.as_float = s0.as_float*s1.as_float;
		}
		else{
			product.as_float = (sig1 < 0.00001) ? 0.0f : sign1*ldexp(sig0, (exp0+exp1+extra-1));
			FPU_hit_cnt[si_isa_V_MUL_F32]++;
		}
		if(sig0 == 0 || sig1 == 0){
	...
	//FPU_cnt[si_isa_V_ADD_F32] ++;
	FPU_cnt[si_isa_V_MUL_F32]++;
/*
	s_0 = abs((int)(s0.as_float * (1<<shift_val)));
	s_1 = abs((int)(s1.as_float * (1<<shift_val)));

	first_bit_pos = -1;
	second_bit_pos = -1;
	i = 31;
	while(i >= 0){
		if(s_0 & (1<<i) && first_bit_pos < 0){
			first_bit_pos = i;
		}
		else if(s_0 & (1<<i))
		{
	...
	if(first_bit_pos < 0){
		...
		if(second_bit_pos < 0){
			if(first_bit_pos-shift_val < 0){
				product.as_float = ((float)sign0*sign1*abs(s_1 >> abs(first_bit_pos-shift_val)))/((float)(1<<shift_val));
			}
			else{
				product.as_float = ((float)sign0*sign1*abs(s_1 << first_bit_pos-shift_val))/((float)(1<<shift_val));
			}
		}
		else if(first_bit_pos - second_bit_pos == 1){
			if(first_bit_pos+1-shift_val < 0){
				product.as_float = ((float)sign0*sign1*abs(s_1 >> abs(first_bit_pos+1-shift_val)))/((float)(1<<shift_val));
			}
			else{
				product.as_float = ((float)sign0*sign1*abs(s_1 << first_bit_pos+1-shift_val))/((float)(1<<shift_val));
			}
		}
		else{
			if(first_bit_pos-shift_val < 0){
	...
	//product.as_float = s0.as_float*s1.as_float;
	//product.as_float = s0.as_float * s1.as_float;
	//printf("S0: %f, S1: %f, Computed Value: %f, Actual Value: %f\n",s0.as_float, s1.as_float, product.as_float, s0.as_float*s1.as_float);
	
	if(product.as_float != s0.as_float*s1.as_float){
	//printf("S0: %f, S1: %f, Computed Value: %f, Actual Value: %f Man0: %f, Exp0: %d, Man1: %f , Exp1: %d\n",s0.as_float, s1.as_float, product.as_float, s0.as_float*s1.as_float, sig0, exp0, sig1, exp1);
	//printf("Extra: %d, Distance %f\n", extra, distance);
	//printf("%f\n", 2*distance);
	}

	//printf("%f, %f, %f, %f, %f, %d, %d\n",s0.as_float, s1.as_float, product.as_float, sig0, sig1, exp0, exp1);
	...
	struct si_inst_t *inst)
{
    ...
    if (PROFILING > 0) {
        ...
        if (!find) {
            if (sqrt_index > fifo_length)
                fprintf (file_unmapped, "SQRT fifo is out of space ...\n");
            else {
                _sqrt[sqrt_index].op0 = src;
                _sqrt[sqrt_index].q = srt.as_float;
                _sqrt[sqrt_index].cnt = 1;
                sqrt_index++;
            }
        }
    }
    else {
        for (unsigned int i = 0; i <= sqrt_index; i++) {
            if (hamm_dis(_sqrt[i].op0,src) < HAMMING_THR) {
                srt.as_float = _sqrt[i].q;
                FPU_hit_cnt[si_isa_V_SQRT_F32] ++;
                break;
            }
        }
...
//DC
	...
	int sign0 = (sig0 < 0) ? -1 : 1;
	int sign1 = (sig1 < 0) ? -1 : 1;
	result.as_float = (sig0 == 0.0f) ? dst.as_float : sign0*ldexp(sig1, (exp0+exp1-1)) + dst.as_float;
	result1.as_float = s0.as_float * s1.as_float + dst.as_float;
	result.as_float = s0.as_float * s1.as_float + dst.as_float;

//printf("%f, %f, %f, %f, %f, %f, %d, %d\n",s0.as_float, s1.as_float, dst.as_float, result.as_float, sig0, sig1, exp0, exp1);
if(result1.as_float != result.as_float){
	//printf("MAC---- %f, %f, %f, %f, %f, %f, %d, %d, %f, %d\n",result1.as_float,result.as_float,s0.as_float,s1.as_float,dst.as_float,sig0,exp0,sign1,sig1,exp1);
}
...
//----------------------
	...
	sig0 = frexp(s0.as_float, &exp0);
	sig1 = frexp(s1.as_float, &exp1);
	sign0 = (sig0 < 0) ? -1 : 1;
	sign1 = (sig1 < 0) ? -1 : 1;

	if(s0.as_float == 0 || s1.as_float == 0){
		result.as_float = 0.0f;
		FPU_hit_cnt[si_isa_V_MAC_F32] ++;
	}
	else if(fabsf(sig0) == 0.5f){
		//product.as_float = s0.as_float*s1.as_float;
		result.as_float = sign0*ldexp(sig1, (exp0+exp1-1));
		FPU_hit_cnt[si_isa_V_MAC_F32] ++;
	}
	else if(fabsf(sig1) == 0.5f){
		//product.as_float = s0.as_float*s1.as_float;
		result.as_float = sign1*ldexp(sig0, (exp0+exp1-1));
		FPU_hit_cnt[si_isa_V_MAC_F32] ++;
	}
	else{
		sig1 = fabsf(sig1);
		sig0 = fabsf(sig0);
		if(abs(sig1-0.75) > abs(sig0-0.75)){
			float temp = sig1;
			sig1 = sig0;
			sig0 = temp;
		}else{
		...
		if(distance > TCAM_DIST){
			...
			int second_bit_1 = -1;
			int current_pos = 23;
			//while((first_bit_0 ^ 0x1<<current_pos) == 0);
			result.as_float = s0.as_float*s1.as_float + dst.as_float;

		}
		else{
			result.as_float = (sig1 == 0.0f) ? 0.0f : sign1*ldexp(sig0, (exp0+exp1+extra-1));
			result.as_float += dst.as_float;
			FPU_hit_cnt[si_isa_V_MAC_F32]++;
		}
		if(sig0 == 0.0f || sig1 == 0.0f){
			//FPU_hit_cnt[si_isa_V_ADD_F32] ++;
...
*/
    ...
    float src0 = s0.as_float;
    float src1 = s1.as_float;
    float src2 = dst.as_float;

    if (PROFILING > 0) {
        bool find = false;
        for (unsigned int i = 0; i < fifo_length; i++) {
            if ((muladd[i].op0 == src0 && muladd[i].op1 == src1 && muladd[i].op2 == src2) ||
                    (muladd[i].op0 == src1 && muladd[i].op1 == src0 && muladd[i].op2 == src2)) {
                find = true;
                muladd[i].cnt ++;
                break;
            }
        ...
        if (!find) {
            if (muladd_index > fifo_length)
                fprintf (file_unmapped, "MULADD fifo is out of space ...\n");
            else {
                muladd[muladd_index].op0 = src0;
                muladd[muladd_index].op1 = src1;
                muladd[muladd_index].op2 = src2;
                muladd[muladd_index].q = result.as_float;
                muladd[muladd_index].cnt = 1;
                muladd_index++;
            }
        }
    }
    else {
        for (unsigned int i = 0; i <= muladd_index; i++) {
            if ((hamm_dis(muladd[i].op0,src0) + hamm_dis(muladd[i].op1,src1) + hamm_dis(muladd[i].op2,src2) <= HAMMING_THR) ||
                (hamm_dis(muladd[i].op0,src1) + hamm_dis(muladd[i].op1,src0) + hamm_dis(muladd[i].op2,src2) <= HAMMING_THR)) {
                FPU_hit_cnt[si_isa_V_MAC_F32] ++;
                result.as_float = muladd[i].q;
                break;
            }
        }
	...
	struct si_inst_t *inst)
{
	...
	bool f1;
	bool f2;
	f0 = _ABS_si_isa_read_reg_dirty(work_item, INST.src0);
	f1 = _ABS_si_isa_read_reg_dirty(work_item, INST.src1);
	f2 = _ABS_si_isa_read_reg_dirty(work_item, INST.src2);
	if (! _ABS_check_dirtiness(f0, f1, f2))
		return;
#endif 
	union si_reg_t s0;
	union si_reg_t s1;
...
//DC
	...
	float sig1 = frexp(s1.as_float, &exp1);
	int sign1 = (sig1 < 0) ? -1 : 1;
	result.as_float = (sig1 == 0.0f) ? s2.as_float : sign1*ldexp(sig0, (exp0+exp1-1)) + s2.as_float;
	result1.as_float = s0.as_float * s1.as_float + s2.as_float;
	result.as_float = s0.as_float * s1.as_float + s2.as_float;

//printf("%f, %f, %f, %f, %f, %f, %d, %d\n",s0.as_float, s1.as_float, s2.as_float, result.as_float, sig0, sig1, exp0, exp1);
if(result1.as_float != result.as_float){
	//printf("MAD---- %f, %f, %f, %f, %f, %f, %d, %d, %f, %d\n",result1.as_float,result.as_float,s0.as_float,s1.as_float,s2.as_float,sig0,exp0,sign1,sig1,exp1);
}

#ifdef _ABS_APPROX
    float src0 = s0.as_float;
    float src1 = s1.as_float;
    float src2 = s2.as_float;

    if (PROFILING > 0) {
        bool find = false;
        for (unsigned int i = 0; i < fifo_length; i++) {
            if ((mad[i].op0 == src0 && mad[i].op1 == src1 && mad[i].op2 == src2) ||
                (mad[i].op0 == src1 && mad[i].op1 == src0 && mad[i].op2 == src2)) {
                find = true;
                mad[i].cnt ++;
                break;
            }
        ...
        if (!find) {
            if (mad_index > fifo_length)
                fprintf (file_unmapped, "MAD fifo is out of space ...\n");
            else {
                mad[mad_index].op0 = src0;
                mad[mad_index].op1 = src1;
                mad[mad_index].op2 = src2;
                mad[mad_index].q = result.as_float;
                mad[mad_index].cnt = 1;
                mad_index++;
            }
        }
        ...
        for (unsigned int i = 0; i <= mad_index; i++) {
            if ((hamm_dis(mad[i].op0,src0) + hamm_dis(mad[i].op1,src1) + hamm_dis(mad[i].op2,src2) <= HAMMING_THR) ||
                (hamm_dis(mad[i].op0,src1) + hamm_dis(mad[i].op1,src0) + hamm_dis(mad[i].op2,src2) <= HAMMING_THR)) {
                FPU_hit_cnt[si_isa_V_MAD_F32] ++;
                result.as_float = mad[i].q;
                break;
            }
        }
	...
	struct si_inst_t *inst)
{
	...
	if (INST.src0 == 0xFF)
		...
		f0 = _ABS_si_isa_read_reg_dirty(work_item, INST.src0);
	f1 = _ABS_si_isa_read_vreg_dirty(work_item, INST.vsrc1);
	if (! _ABS_check_dirtiness(f0, f1, false))
		return;
#endif 
	union si_reg_t s0;
	union si_reg_t s1;
	...
	/* Write the results. */
#ifdef _ABS_REUSE
        _ABS_si_isa_check_v2ops (work_item, INST.vdst, dif.as_uint, s0.as_float, s1.as_float, si_isa_V_SUB_F32, VREG);
#else   
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt [si_isa_V_SUB_F32]++;
    //printf ("si_isa_V_SUB_F32\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
    _ABS_si_isa_check_v2ops (work_item, INST.vdst, dif.as_uint, s1.as_float, s0.as_float, si_isa_V_SUBREV_F32, VREG);
#else
	si_isa_write_vreg(work_item, INST.vdst, dif.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_SUBREV_F32]++;
    //printf ("si_isa_V_SUBREV_F32\n");
#endif
...
//DC
		...
		float sig1 = frexp(s1.as_float, &exp1);
		int sign1 = (sig1 < 0) ? -1 : 1;
		product.as_float = (sig1 == 0.0f) ? 0.0f : sign1*ldexp(sig0, (exp0+exp1-1));
		product.as_float = s0.as_float * s1.as_float;
	}

	/* Write the results. */
#ifdef _ABS_REUSE
        _ABS_si_isa_check_v2ops (work_item, INST.vdst, product.as_uint, s0.as_float, s1.as_float, si_isa_V_MUL_LEGACY_F32, VREG);
#else
	si_isa_write_vreg(work_item, INST.vdst, product.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_MUL_LEGACY_F32]++;
    //printf ("si_isa_V_MUL_LEGACY_F32\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
        _ABS_si_isa_check_v2ops (work_item, INST.vdst, min.as_uint, s0.as_float, s1.as_float, si_isa_V_MIN_F32, VREG);
#else
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
        _ABS_si_isa_check_v2ops (work_item, INST.vdst, max.as_uint, s0.as_float, s1.as_float, si_isa_V_MAX_F32, VREG);
#else
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
        _ABS_si_isa_check_v3ops (work_item, INST.vdst, dst.as_uint, s0.as_float, K.as_float, s1.as_float, si_isa_V_MADMK_F32);
#else
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, SI_VCC, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_LT_F32, SREG);
#else
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, SI_VCC, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_GT_F32, SREG);
#else
	si_isa_bitmask_sreg(work_item, SI_VCC, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, SI_VCC, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_NGT_F32, SREG);
#else
	si_isa_bitmask_sreg(work_item, SI_VCC, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, SI_VCC, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_NEQ_F32, SREG);
#else
	si_isa_bitmask_sreg(work_item, SI_VCC, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, sum.as_uint, s0.as_float, s1.as_float, si_isa_V_ADD_F32_VOP3a, VREG);
#else
	si_isa_write_vreg(work_item, INST.vdst, sum.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_ADD_F32_VOP3a]++;
    //printf ("si_isa_V_ADD_F32_VOP3a\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, diff.as_uint, s1.as_float, s0.as_float, si_isa_V_SUBREV_F32_VOP3a, VREG);
#else
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_SUBREV_F32_VOP3a]++;
    //printf ("si_isa_V_SUBREV_F32_VOP3a\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	float sig0 = frexp(s0.as_float, &exp0);
	float sig1 = frexp(s1.as_float, &exp1);
	result.as_float = (sig1 == 0.0f) ? 0.0f : ldexp(sig0, (exp0+exp1-1));
	//result.as_float = s0.as_float * s1.as_float;

	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_MUL_F32_VOP3a, VREG);
#else
	si_isa_write_vreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_MUL_F32_VOP3a]++;
    //printf ("si_isa_V_MUL_F32_VOP3a\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_MAX_F32_VOP3a, VREG);
#else
	si_isa_write_vreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v3ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, s2.as_float, si_isa_V_FMA_F32);
#else
	si_isa_write_vreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_FMA_F32] ++;
    //printf ("si_isa_V_FMA_F32\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, 0.0f, si_isa_V_FRACT_F32_VOP3a, VREG);
#else
	si_isa_write_vreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
...
#ifdef _ABS_PRINTING_ISA
    FPU_cnt[si_isa_V_FRACT_F32_VOP3a]++;
    //printf ("si_isa_V_FRACT_F32_VOP3a\n");
#endif
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_LT_F32_VOP3a, SREG);
#else
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_EQ_F32_VOP3a, SREG);
#else
	si_isa_bitmask_sreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_GT_F32_VOP3a, SREG);
#else
	si_isa_bitmask_sreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_NEQ_F32_VOP3a, SREG);
#else
	si_isa_bitmask_sreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
	...
	struct si_inst_t *inst)
{
	...
	/* Write the results. */
#ifdef _ABS_REUSE
	_ABS_si_isa_check_v2ops (work_item, INST.vdst, result.as_uint, s0.as_float, s1.as_float, si_isa_V_CMP_NLT_F32_VOP3a, SREG);
#else
	si_isa_bitmask_sreg(work_item, INST.vdst, result.as_uint);
#endif
	/* Print isa debug information. */
