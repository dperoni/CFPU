...
static char *m2s_help =
		...
		"      GPU model.\n"
		"\n"
        "  --si-fifo-length {0-10000}\n"
        "      Length of the FIFO\n"
        "\n"
        "  --si-profiling {0-fifo_length}\n"
        "      Length of profiling for the FIFO\n"
        "\n"
        "  --si-hamming-dist {0-32}\n"
        "      Max Hamming distance during matching \n"
        "\n"
		"\n"
		"================================================================================\n"
		"ARM CPU Options\n"
...
static void m2s_read_command_line(int *argc_ptr, char **argv)
{
	...
	for (argi = 1; argi < argc; argi++)
	{
        ...
        //_ABS_
        /* set the fifo length */
        if (!strcmp(argv[argi], "--si-fifo-length"))
        {
            m2s_need_argument(argc, argv, argi);
            fifo_length = atoi(argv[++argi]);
            continue;
        }

        if (!strcmp(argv[argi], "--si-profiling"))
        {
            m2s_need_argument(argc, argv, argi);
            PROFILING = atoi(argv[++argi]);
            continue;
        }
        
        if(!strcmp(argv[argi], "--si-cfpu_max_error"))
        {
            m2s_need_argument(argc, argv, argi);
            //fprintf(stderr,"here\n");
            CFPU_MAX_ERROR = atof(argv[++argi]);
            fprintf(stderr,"CFPU MAX ERROR : %lf\n", CFPU_MAX_ERROR);
            continue;
        }
	if(!strcmp(argv[argi], "--si-cfpu_mode"))
        {
            m2s_need_argument(argc, argv, argi);
            //fprintf(stderr,"here\n");
            CFPU_MODE = atof(argv[++argi]);
            fprintf(stderr,"CFPU MODE : %d\n", CFPU_MODE);
            continue;
        }
        if(!strcmp(argv[argi], "--si-tcam-rew"))
        {
            m2s_need_argument(argc, argv, argi);
            //fprintf(stderr,"here\n");
            TCAM_REW = atof(argv[++argi]);
            fprintf(stderr,"TCAM Rewrite : %lf\n", TCAM_REW);
            continue;

        }
	if(!strcmp(argv[argi], "--si-abuff-size"))
        {
            m2s_need_argument(argc, argv, argi);
            ABUFF_SIZE = atoi(argv[++argi]);
            fprintf(stderr,"abuff-size : %d\n", ABUFF_SIZE);
            continue;

        }

        if (!strcmp(argv[argi], "--si-hamming-dist"))
        {
            m2s_need_argument(argc, argv, argi);
            HAMMING_THR = atoi(argv[++argi]);
            continue;
        }
...
binary_name = getenv("M2S_OPENCL_BINARY");
	if (!binary_name || !*binary_name){
		printf("read error, no M2S_OPENCL_BINARY defined\n");
	}
	else{
	/* Load binary */
	printf("Opening %s\n", binary_name);
...
static void _ABS_dump_summary()
{
	printf ("\n[ _ABS_dump_summary ]\n");
	printf ("I(#)\t gated_I(#)\t gating(%)\n");
	unsigned long tot_inst_cnt = 0;
    unsigned long tot_gated_inst_cnt = 0;
    for (int i = 0; i < si_isa_LAST; i++) {
		printf ("%u\t\t %u\t\t %2f\n", inst_cnt[i], gated_inst_cnt[i], ((float)gated_inst_cnt[i]/((float)inst_cnt[i]+1))*100);
	    tot_inst_cnt += inst_cnt[i];
        tot_gated_inst_cnt += gated_inst_cnt[i];
    }
	printf ("%u\t\t %u\t\t %2f\n", tot_inst_cnt, tot_gated_inst_cnt, ((float)tot_gated_inst_cnt/((float)tot_inst_cnt+1))*100);
	
    printf ("check_wr=%u\t clean_wr=%u\t percentage=%2f\n", gated_vreg_cnt, clean_cnt, ((float)gated_vreg_cnt/((float)vreg_cnt+1))*100);
	printf ("check=%u\t clean=%u\t percentage=%2f\n", check_cnt, clean_cnt, ((float)clean_cnt/((float)check_cnt+1))*100);
	printf ("\n");
}
...
void load_tcam()
{
    ...
    if (PROFILING > 0) {
        printf ("Profiling ... \n");
        file_unmapped = fopen ("unmapped.rep", "w");
    }
    else 
    {
        printf ("------- start loading tcam ------ \n");
        FILE* file_add = fopen ("add.txt", "r");
        float src0, src1, q;
        unsigned long cnt;

        if (file_add == NULL)
            printf ("CANNOT open add.txt\n");
        else {
            while (fscanf (file_add, "%e\t%e\t%e\t%d\n", &src0, &src1, &q, &cnt) == 4)
            {
                int flag_find = 0;
                for (unsigned int i = 0; i < add_index; i++) {
                    ...
                    if ((op0 == src0 && op1 == src1) || (op1 == src0 && op0 == src1)) {
                        flag_find = 1;
                        break;
                    }
                }
                ...
                if (!flag_find) {
                    ...
                    //else
                    //    printf ("OUT OF FIFO during loading add: %u \n", add_index);
                }
            }
            //printf ("Loaded %u values for adder \n", add_index);
        }
        fclose(file_add);


        FILE* file_mul = fopen ("mul.txt", "r");
        if (file_mul == NULL)
            printf ("CANNOT open mul.txt\n");
        else {
            while (fscanf (file_mul, "%e\t%e\t%e\t%d\n", &src0, &src1, &q, &cnt) == 4)
            {
                int flag_find = 0;
                for (unsigned int i = 0; i < mul_index; i++) {
                    float op0 = mul[i].op0;
                    float op1 = mul[i].op1;
                ...
                if (!flag_find) {
                    if (mul_index < fifo_length) {
                        mul[mul_index].op0 = src0;
                        mul[mul_index].op1 = src1;
                        mul[mul_index].q = q;
                        mul_index ++;
                    }
                    //else
                    //    printf ("OUT OF FIFO during loading mul: %u \n", mul_index);
                }
            }
           // printf ("Loaded %u values for multiplier \n", mul_index);
        }
        fclose(file_mul);

        FILE* file_muladd = fopen ("muladd.txt", "r");
        float src2;
        if (file_muladd == NULL)
            printf ("CANNOT open muladd.txt\n");
        else {
            while (fscanf (file_muladd, "%e\t%e\t%e\t%e\t%d\n", &src0, &src1, &src2, &q, &cnt) == 5)
            {
                int flag_find = 0;
                for (unsigned int i = 0; i < muladd_index; i++) {
                    float op0 = muladd[i].op0;
                    float op1 = muladd[i].op1;
                    float op2 = muladd[i].op2;
                    if ((op0 == src0 && op1 == src1 && op2 == src2) || (op0 == src1 && op1 == src0 && op2 == src2)) {
                        flag_find = 1;
                        break;
                    }
                }
                if (!flag_find) {
                    if (muladd_index < fifo_length) {
                        ...
                        muladd[muladd_index].q = q;
                        muladd_index ++;
                    }
                    //else
                    //    printf ("OUT OF FIFO during loading muladd: %u \n", muladd_index);
                }
            }
            //printf ("Loaded %u values for muladdtiplier \n", muladd_index);
        }
        fclose(file_muladd);

 
        FILE* file_mad = fopen ("mad.txt", "r");
        if (file_mad == NULL)
            printf ("CANNOT open mad.txt\n");
        else {
            while (fscanf (file_mad, "%e\t%e\t%e\t%e\t%d\n", &src0, &src1, &src2, &q, &cnt) == 5)
            {
                int flag_find = 0;
            ...
            //printf ("Loaded %u values for mad \n", mad_index);
        }
        fclose(file_mad);
        
       
        /*
        FILE* file_recip = fopen ("recip.txt", "r");
        if (file_recip == NULL)
            printf ("CANNOT open recip.txt\n");
        else {
            while (fscanf (file_recip, "%e\t%d\n", &src0, &cnt) == 2)
            {
                int flag_find = 0;
                for (unsigned int i = 0; i < recip_index; i++) {
                    float op0 = recip[i].op0;
                ...
                if (!flag_find) {
                    if (recip_index < fifo_length) {
                        recip[recip_index].op0 = src0;
                        recip_index ++;
                    }
                    //else
                    //    printf ("OUT OF FIFO during loading recip: %u \n", recip_index);
                }
            }
            printf ("Loaded %u values for recip \n", recip_index);
        }
        fclose(file_recip);

        FILE* file_flp2int = fopen ("flp2int.txt", "r");
        if (file_flp2int == NULL)
            printf ("CANNOT open flp2int.txt\n");
        else {
            while (fscanf (file_flp2int, "%e\t%d\n", &src0, &cnt) == 2)
            {
                int flag_find = 0;
                for (unsigned int i = 0; i < flp2int_index; i++) {
                    float op0 = flp2int[i].op0;
                ...
                if (!flag_find) {
                    if (flp2int_index < fifo_length) {
                        flp2int[flp2int_index].op0 = src0;
                        flp2int_index ++;
                    }
                    //else
                    //    printf ("OUT OF FIFO during loading flp2int: %u \n", flp2int_index);
                }
            }
            printf ("Loaded %u values for flp2int \n", flp2int_index);
        }
        fclose(file_flp2int);
        */
        FILE* file_sqrt = fopen ("sqrt.txt", "r");
        if (file_sqrt == NULL)
            printf ("CANNOT open sqrt.txt\n");
        else {
            while (fscanf (file_sqrt, "%e\t%e\t%d\n", &src0, &q, &cnt) == 3) 
            {
                int flag_find = 0;
                for (unsigned int i = 0; i < sqrt_index; i++) {
                    float op0 = _sqrt[i].op0;
                ...
                if (!flag_find) {
                    if (sqrt_index < fifo_length) {
                        _sqrt[sqrt_index].op0 = src0;
                        _sqrt[sqrt_index].q = q;
                        sqrt_index ++;
                    }
                    //else
                    //    printf ("OUT OF FIFO during loading sqrt: %u \n", sqrt_index);
                }
            }
           // printf ("Loaded %u values for sqrt \n", sqrt_index);
        }
        fclose(file_sqrt);
    }
}
...
void dump_statistics()
{
    ...
    {   
	printf("Prof\n");
        FILE* file_add = fopen ("add.txt", "w");
        FILE* file_mul = fopen ("mul.txt", "w");
        FILE* file_muladd = fopen ("muladd.txt", "w");
        FILE* file_sqrt = fopen ("sqrt.txt", "w");
        FILE* file_mad = fopen ("mad.txt", "w");
        //FILE* file_recip = fopen ("recip.txt", "w");
        //FILE* file_flp2int = fopen ("flp2int.txt", "w");
	
        for (unsigned int x = 0; x < fifo_length; x++) {
            for (unsigned int y = 0; y < fifo_length; y++) {
                if (add[y].cnt < add[y + 1].cnt) {
                    unsigned long cnt = add[y + 1].cnt;
                    float op0 = add[y + 1].op0;
                    float op1 = add[y + 1].op1;
                    float q = add[y + 1].q;
                    add[y + 1].cnt = add[y].cnt;
                    add[y + 1].op0 = add[y].op0;
                    add[y + 1].op1 = add[y].op1;
                    add[y + 1].q = add[y].q;
                    add[y].cnt = cnt;
                    add[y].op0 = op0;
                    add[y].op1 = op1;
                ...
                if (mul[y].cnt < mul[y + 1].cnt) {
                    unsigned long cnt = mul[y + 1].cnt;
                    float op0 = mul[y + 1].op0;
                    float op1 = mul[y + 1].op1;
                    float q = mul[y + 1].q;
                    mul[y + 1].cnt = mul[y].cnt;
                    mul[y + 1].op0 = mul[y].op0;
                    mul[y + 1].op1 = mul[y].op1;
                    mul[y + 1].q = mul[y].q;
                    mul[y].cnt = cnt;
                    mul[y].op0 = op0;
                    mul[y].op1 = op1;
                ...
                if (muladd[y].cnt < muladd[y + 1].cnt) {
                    unsigned long cnt = muladd[y + 1].cnt;
                    float op0 = muladd[y + 1].op0;
                    float op1 = muladd[y + 1].op1;
                    float op2 = muladd[y + 1].op2;
                    float q = muladd[y + 1].q;
                    muladd[y + 1].cnt = muladd[y].cnt;
                    muladd[y + 1].op0 = muladd[y].op0;
                    muladd[y + 1].op1 = muladd[y].op1;
                    muladd[y + 1].op2 = muladd[y].op2;
                    muladd[y + 1].q = muladd[y].q;
                    muladd[y].cnt = cnt;
                    muladd[y].op0 = op0;
                    muladd[y].op1 = op1;
                    muladd[y].op2 = op2;
                ...
                if (mad[y].cnt < mad[y + 1].cnt) {
                    unsigned long cnt = mad[y + 1].cnt;
                    float op0 = mad[y + 1].op0;
                    float op1 = mad[y + 1].op1;
                    float op2 = mad[y + 1].op2;
                    float q = mad[y + 1].q;
                    mad[y + 1].cnt = mad[y].cnt;
                    mad[y + 1].op0 = mad[y].op0;
                    mad[y + 1].op1 = mad[y].op1;
                    mad[y + 1].op2 = mad[y].op2;
                    mad[y + 1].q = mad[y].q;
                    mad[y].cnt = cnt;
                    mad[y].op0 = op0;
                    mad[y].op1 = op1;
                ...
                if (_sqrt[y].cnt < _sqrt[y + 1].cnt) {
                    unsigned long cnt = _sqrt[y + 1].cnt;
                    float op0 = _sqrt[y + 1].op0;
                    float q = _sqrt[y + 1].q;
                    _sqrt[y + 1].cnt = _sqrt[y].cnt;
                    _sqrt[y + 1].op0 = _sqrt[y].op0;
                    _sqrt[y + 1].q = _sqrt[y].q;
                    _sqrt[y].cnt = cnt;
                    _sqrt[y].op0 = op0;
                    _sqrt[y].q = q;
                }
    ...
    printf("Current working dir: %s\n", cwd);
        fprintf(file_add, "woop");
	for (unsigned int sh = 0; sh < PROFILING; sh++) {
            fprintf (file_add, "%.32e\t%.32e\t%.32e\t%u\n", add[sh].op0, add[sh].op1, add[sh].q, add[sh].cnt);
            fprintf (file_mul, "%.32e\t%.32e\t%.32e\t%u\n", mul[sh].op0, mul[sh].op1, mul[sh].q, mul[sh].cnt);
            fprintf (file_muladd, "%.32e\t%.32e\t%.32e\t%.32e\t%u\n", muladd[sh].op0, muladd[sh].op1, muladd[sh].op2, muladd[sh].q, muladd[sh].cnt);
            fprintf (file_mad, "%.32e\t%.32e\t%.32e\t%.32e\t%u\n", mad[sh].op0, mad[sh].op1, mad[sh].op2, mad[sh].q, mad[sh].cnt);
            fprintf (file_sqrt, "%.32e\t%.32e\t%u\n", _sqrt[sh].op0, _sqrt[sh].q, _sqrt[sh].cnt);
            //fprintf (file_recip, "%.32e\t%u\n", recip[sh].op0, recip[sh].cnt);
            //fprintf (file_flp2int, "%.32e\t%u\n", flp2int[sh].op0, flp2int[sh].cnt);
        }
        fclose (file_add);
        fclose (file_mul);
        fclose (file_muladd);
        ...
        //fclose (file_recip);
        //fclose (file_flp2int);
        fclose (file_unmapped);
        printf("\n-------- DONE with writing profiles ----------\n");
    }
    //else
    //{
	printf("\n-------- FIRST STAGE CFPU HIT STATISTICS ----------\n");
        printf("\tHits_S1 \tHit_S2 \tTotal \tHit rate \tStage 2 Hit rate \tOverall Hit rate\n");
	printf("test %d\n", si_isa_V_MUL_F32);
        for (int i = 0; i < si_last_F32; i++)  
	    if(i == 1){
		printf("FPU_ID_MUL: %d\t %d\t %d\t %.4f\t %.4f\t %.4f\n", FPU_hit_cnt[i], FPU_rew_cnt[i], FPU_cnt[i], ((float)FPU_hit_cnt[i]/(float)FPU_cnt[i])*100, ((float)FPU_rew_cnt[i]/(float)FPU_cnt[i])*100,(((float)FPU_rew_cnt[i]+(float)FPU_hit_cnt[i])/(float)FPU_cnt[i])*100);
	    }
	    else{
            printf("FPU_ID_%d: %d\t %d\t %d\t %.4f\t %.4f\t %.4f\n", i, FPU_hit_cnt[i], FPU_rew_cnt[i], FPU_cnt[i] , ((float)FPU_hit_cnt[i]/(float)FPU_cnt[i])*100, ((float)FPU_rew_cnt[i]/(float)FPU_cnt[i])*100,(((float)FPU_rew_cnt[i]+(float)FPU_hit_cnt[i])/(float)FPU_cnt[i])*100);
	}	
        printf("\n");


     /*   printf("\n-------- SECOND STAGE CFPU HIT STATISTICS ----------\n");
        printf("\tCFPU_hit_cnt\t CFPU_cnt\t hit rate\n");
        for (int i = 0; i < si_last_F32; i++)
	    if(i == 1){
	printf("FPU_ID_MUL: %d\t %d\t %f\n", FPU_hit_cnt[i], FPU_cnt[i], ((float)FPU_hit_cnt[i]/(float)FPU_cnt[i])*100);
}
	else{ 
            printf("FPU_ID_%d: %d\t %d\t %f\n", i, FPU_hit_cnt[i], FPU_cnt[i], ((float)FPU_hit_cnt[i]/(float)FPU_cnt[i])*100);
}
        printf("\n");
*/
    //}
